"""
LATEX model
添加**抑制无效信息**（Damping Noise），及语义滤波（Semantic Filtering）
"""
...
class DINO_HDLayout(nn.Module):
    ...
    def forward(self, x):
        ...
        region_logits = region_class
        region_probs = region_logits.softmax(dim=-1)
        objectness_score = 1.0 - region_probs[..., 0:1]
        hs_block_guided = hs_block * objectness_score
