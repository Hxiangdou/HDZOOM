...
from ..common.lora import LoRALinearLayer
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # lora for subject
        self.subject_q_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for subject to_q linear
        self.subject_k_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for subject to_k linear
        self.subject_v_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for subject to_v linear

        # lora for object
        self.object_q_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for object to_q linear
        self.object_k_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for object to_k linear
        self.object_v_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for object to_v linear
...
class DesignFluxAttnProcessor2_0:
    """Attention processor used typically in processing the SD3-like self-attention projections."""
    ...
    def __call__(
        ...
        if subject_hidden_states is not None:
            subject_hidden_states_query = attn.to_q(subject_hidden_states) + attn.lora_weight * attn.subject_q_lora(subject_hidden_states)
            subject_hidden_states_key = attn.to_k(subject_hidden_states) + attn.lora_weight * attn.subject_k_lora(subject_hidden_states)
            subject_hidden_states_value = attn.to_v(subject_hidden_states) + attn.lora_weight * attn.subject_v_lora(subject_hidden_states)
            subject_hidden_states_query = subject_hidden_states_query.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            subject_hidden_states_key = subject_hidden_states_key.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            subject_hidden_states_value = subject_hidden_states_value.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            if attn.norm_q is not None:
                subject_hidden_states_query = attn.norm_q(subject_hidden_states_query)
            if attn.norm_k is not None:
                subject_hidden_states_key = attn.norm_k(subject_hidden_states_key)
        if subject_rotary_emb is not None:
            subject_hidden_states_query = apply_rotary_emb(subject_hidden_states_query, subject_rotary_emb)
            subject_hidden_states_key = apply_rotary_emb(subject_hidden_states_key, subject_rotary_emb)

        # handle object_hidden_states
        if object_bbox_hidden_states is not None:
            if encoder_hidden_states is not None: # double transformer block
                object_bbox_hidden_states_query = attn.add_q_proj(object_bbox_hidden_states) + attn.lora_weight * attn.object_q_lora(object_bbox_hidden_states)
                object_bbox_hidden_states_key = attn.add_k_proj(object_bbox_hidden_states) + attn.lora_weight * attn.object_k_lora(object_bbox_hidden_states)
                object_bbox_hidden_states_value = attn.add_v_proj(object_bbox_hidden_states) + attn.lora_weight * attn.object_v_lora(object_bbox_hidden_states)
                object_bbox_hidden_states_query = object_bbox_hidden_states_query.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                object_bbox_hidden_states_key = object_bbox_hidden_states_key.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                object_bbox_hidden_states_value = object_bbox_hidden_states_value.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                if attn.norm_added_q is not None:
                    object_bbox_hidden_states_query = attn.norm_added_q(object_bbox_hidden_states_query)
                if attn.norm_added_k is not None:
                    object_bbox_hidden_states_key = attn.norm_added_k(object_bbox_hidden_states_key)
            
            else: # single transformer block
                object_bbox_hidden_states_query = attn.to_q(object_bbox_hidden_states) + attn.lora_weight * attn.object_q_lora(object_bbox_hidden_states)
                object_bbox_hidden_states_key = attn.to_k(object_bbox_hidden_states) + attn.lora_weight * attn.object_k_lora(object_bbox_hidden_states)
                object_bbox_hidden_states_value = attn.to_v(object_bbox_hidden_states) + attn.lora_weight * attn.object_v_lora(object_bbox_hidden_states)
                ...
                if attn.norm_q is not None:
                    object_bbox_hidden_states_query = attn.norm_q(object_bbox_hidden_states_query)
                if attn.norm_k is not None:
                    object_bbox_hidden_states_key = attn.norm_k(object_bbox_hidden_states_key)
            
            
        if object_rotary_emb is not None:
            object_bbox_hidden_states_query = apply_rotary_emb(object_bbox_hidden_states_query, object_rotary_emb)
            object_bbox_hidden_states_key = apply_rotary_emb(object_bbox_hidden_states_key, object_rotary_emb)

        # concat subject after query
        if subject_hidden_states is not None:
            query = torch.cat([query, subject_hidden_states_query], dim=2)
            key = torch.cat([key, subject_hidden_states_key], dim=2)
            value = torch.cat([value, subject_hidden_states_value], dim=2)
        
        # concat object before query
        if object_bbox_hidden_states is not None:
            query = torch.cat([object_bbox_hidden_states_query, query], dim=2)
            key = torch.cat([object_bbox_hidden_states_key, key], dim=2)
            value = torch.cat([object_bbox_hidden_states_value, value], dim=2)

        # query is [object_bbox_hidden_states_query, encoder_hidden_states_query, query, subject_hidden_states_query]
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        ...
        if encoder_hidden_states is not None: # double transformer block
            # split object first 
            if object_bbox_hidden_states is not None: 
                object_bbox_hidden_states, hidden_states = (
                    hidden_states[:, : object_bbox_hidden_states.shape[1]],
                    hidden_states[:, object_bbox_hidden_states.shape[1] :],
                )
            # then split encoder_hidden_states 
            encoder_hidden_states, hidden_states = (
                hidden_states[:, : encoder_hidden_states.shape[1]],
                hidden_states[:, encoder_hidden_states.shape[1] :],
            )
            # last, split hidden_states and  subject
            if subject_hidden_states is not None:
                # split at split_index
                split_index = hidden_states.shape[1] - subject_hidden_states.shape[1]  
                hidden_states, subject_hidden_states = (
                    hidden_states[:, :split_index],  
                    hidden_states[:, split_index:],  
                )
            ...
            if subject_hidden_states is not None:
                subject_hidden_states = attn.to_out[0](subject_hidden_states)
                subject_hidden_states = attn.to_out[1](subject_hidden_states)
            if object_bbox_hidden_states is not None:
                object_bbox_hidden_states = attn.to_add_out(object_bbox_hidden_states)


            return hidden_states, encoder_hidden_states, subject_hidden_states, object_bbox_hidden_states
        else:
            # split object first
            if object_bbox_hidden_states is not None:
                object_bbox_hidden_states, hidden_states = (
